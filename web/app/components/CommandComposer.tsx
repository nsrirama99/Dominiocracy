...
export default function CommandComposer({ initialText = "", targetLabel, onPreview, onSend }: Props) {
	const [text, setText] = useState(initialText);
	const [urgency, setUrgency] = useState<"low" | "med" | "high">("med");
	const [resources, setResources] = useState<"conserve" | "standard" | "excess">("standard");
	const [personalities, setPersonalities] = useState<string[]>([]);

	const targetChip = useMemo(() => (targetLabel ? (
		<span className="px-2 py-0.5 text-xs bg-zinc-700 text-white rounded-full border border-zinc-600">{targetLabel}</span>
	) : null), [targetLabel]);

	const preview = useMemo(() => {
		if (!text) return null;
		const summary = `Intent: ${text.trim().slice(0, 80)}${text.length > 80 ? "…" : ""}`;
		const misreads = [
	...
	return (
		<div className="fixed left-1/2 -translate-x-1/2 bottom-4 z-40 w-[min(900px,92vw)]">
			<div className="bg-zinc-900/95 backdrop-blur border border-zinc-700 rounded-xl shadow-2xl p-3">
				<div className="flex items-center gap-2 mb-2">
					...
					<button
						...
						disabled={!text}
						onClick={() => onSend?.({ text, urgency, resources, targetLabel, personalities })}
					>
						Issue Order
					</button>
				</div>
				...
				{preview && (
					<div className="mt-3 border-t border-zinc-800 pt-2">
						<div className="text-xs text-zinc-300 mb-1">Preview</div>
